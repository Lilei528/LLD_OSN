...
from utils.mahalanobis_torch import *
class category_datas():
    def __init__(self,n_class,category_keep_num,feat=768):
        '''
        ...
        self.cov_rev=[torch.eye(feat) for i in range(n_class)]
        self.mean=[torch.zeros(feat) for i in range(n_class)]
        self.label_len=n_class
...
d=m_dist(mc.mean[1],x,mc.cov_rev[1])
print(d)'''
